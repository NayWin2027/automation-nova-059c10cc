...
BEGIN
  ...
  -- Effective expiry: admin override wins; else exactly 1 month from start; else expired
  ...
  ELSE
    _effective_expiry := now() - INTERVAL '1 second';
  END IF;
...
BEGIN
  ...
  IF NEW.credits > OLD.credits THEN
    ...
    ELSIF OLD.credits_started_at + INTERVAL '1 month' < now() THEN
      -- Past 1 month: fresh start
      NEW.credits_started_at := now();
    END IF;
    -- If still within 1 month, keep existing credits_started_at (topup merges)
  END IF;
