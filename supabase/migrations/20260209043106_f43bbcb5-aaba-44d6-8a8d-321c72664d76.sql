...
CREATE OR REPLACE FUNCTION public.deduct_user_credits(_user_id uuid, _tool_id text, _is_own_api boolean DEFAULT false)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _is_admin THEN
    -- Log usage for admin but don't deduct
  ...
  -- Get tool credit cost from tool_settings
