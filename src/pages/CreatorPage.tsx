        ...
        {(result || generatedImg) && (
          <div className="animate-in fade-in zoom-in-95 duration-500 space-y-4 max-w-2xl mx-auto">
            <div className="flex justify-between items-center px-4">
              <h3 className="text-[9px] font-black text-primary uppercase tracking-[0.2em]">CONTENT OUTPUT</h3>
              <div className="flex gap-3">
                <button onClick={() => navigator.clipboard.writeText(result)} className="text-[8px] font-black text-muted-foreground hover:text-foreground uppercase tracking-widest transition-colors">COPY</button>
                <button onClick={() => {setResult(''); setGeneratedImg(null);}} className="text-[8px] font-black text-destructive uppercase tracking-widest transition-colors">CLEAR</button>
              </div>
            </div>
            
            <div className="space-y-3">
              {generatedImg && (
                <div className="animate-in zoom-in-95 duration-500 relative group rounded-[32px] overflow-hidden border-2 border-primary/20 shadow-2xl">
                  <img src={generatedImg} alt="AI Generated Illustration" className="w-full object-cover" />
                </div>
              )}
              
              {result && (
                <div className="p-6 bg-card/80 rounded-[36px] border border-primary/10 shadow-2xl relative group">
                  <p className="text-[12px] leading-[1.8] font-medium text-foreground whitespace-pre-wrap">{result}</p>
                </div>
              )}
            </div>
          </div>
