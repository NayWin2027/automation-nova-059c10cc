...
const TranslateView: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (charCount <= 5000) tierIndex = 0;else
    if (charCount <= 10000) tierIndex = 1;else
    if (charCount <= 15000) tierIndex = 2;else
    if (charCount <= 20000) tierIndex = 3;else
    if (charCount <= 25000) tierIndex = 4;else
    tierIndex = 5;
      ...
      <div className="neon-glass rounded-[48px] p-8 md:p-12 space-y-12 border border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.9)] relative overflow-hidden">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            ...
            <button
              onClick={() => setText("")}
              className="font-black text-rose-400 hover:text-rose-300 transition-colors uppercase tracking-widest text-base">
        ...
        <div className="space-y-4 bg-white/5 p-6 rounded-[36px] border border-white/5 shadow-inner">
          <div className="flex justify-between items-center ml-2">
            <h4 className="font-black text-amber-400 tracking-[0.2em] uppercase flex items-center gap-2 premium-font text-lg">
              <Sparkles className="w-3 h-3" /> EMOTION TUNER
            </h4>
            {selectedEmotion === "PROFESSIONAL" &&
            <span className="font-black text-emerald-400 uppercase tracking-widest flex items-center gap-1 text-sm">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            ...
            <button
              ...
              "bg-black/40 border-white/5 text-slate-400 hover:text-slate-200"}`
                ...
                <span className="font-black tracking-tight uppercase premium-font text-sm">{emo.label}</span>
              </button>
            )}
          </div>
        </div>
