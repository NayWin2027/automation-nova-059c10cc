...
export default function VideoRecapView() {
  ...
  const generateAudioFromText = async (text: string, segments: ScriptSegment[]) => {
    ...
    try {
      ...
      // Generate TTS for each segment with robust retry on rate-limit
      const MAX_RETRIES = 5;
      for (let i = 0; i < segments.length; i++) {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            console.error("[RecapVideo] TTS error for segment", i, "attempt", attempt, ttsErr);
          ...
          // If we got real audio (not WebSpeech fallback), success
        ...
        if (!succeeded || !segB64 || segB64.startsWith("WEBSPEECH:")) {
          toast.error("❌ TTS rate limit ကြောင့် မလုပ်နိုင်ပါ — ခဏစောင့်ပြီး ပြန်လုပ်ပါ။");
          setAnalyzing(false);
          return;
        }
  ...
  return (
    <div className="flex flex-col gap-5 pb-32 max-w-lg mx-auto px-2 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between py-2">
        ...
        <h1 className="text-[11px] font-black text-white uppercase tracking-widest">NOVA VIDEO PRECAP
