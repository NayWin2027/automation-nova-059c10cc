...
const voiceDb = {
  getVoiceSettings: async (): Promise<VoiceSettings | null> => {
    const { data } = await supabase.from("app_settings").select("value").eq("key", "voice_settings").maybeSingle();
  ...
  upsertVoiceSettings: async (settings: VoiceSettings) => {
    ...
    if (existing) {
      ...
      await supabase.from("app_settings").insert({ key: "voice_settings", value: settings as never });
    }
  },
};
...
const VoicePage: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      for (let i = 0; i < rawTextChunks.length; i++) {
        currentChunk += rawTextChunks[i];
        if (i % 2 === 1 || i === rawTextChunks.length - 1) {
          if (currentChunk.trim().length > 600 || i === rawTextChunks.length - 1) {
      ...
      if (pcmBuffers.length > 0) {
        let totalLength = 0;
        const byteArrays = pcmBuffers.map((b64) => {
        ...
        let binaryString = "";
        const maxStack = 8192;
        for (let i = 0; i < combined.length; i += maxStack) {
          const chunkArr = Array.from(combined.subarray(i, i + maxStack));
          binaryString += String.fromCharCode.apply(null, chunkArr);
        }
        combinedPcmBase64 = btoa(binaryString);
      }

      const pcmData = combinedPcmBase64;
      if (isError && !pcmData) throw new Error("TTS Generation failed chunking");
      // ----------------------------------------------------
      if (pcmData) {
  ...
  const tiers = [
    {
      label: tierSrc?.tier1Label || dv.tier1Label,
      credits: tierSrc?.tier1Credits || dv.tier1Credits,
      value: tierSrc?.tier1Value || dv.tier1Value,
    },
    {
      label: tierSrc?.tier2Label || dv.tier2Label,
      credits: tierSrc?.tier2Credits || dv.tier2Credits,
      value: tierSrc?.tier2Value || dv.tier2Value,
    },
    {
      label: tierSrc?.tier3Label || dv.tier3Label,
      credits: tierSrc?.tier3Credits || dv.tier3Credits,
      value: tierSrc?.tier3Value || dv.tier3Value,
    },
    {
      label: tierSrc?.tier4Label || dv.tier4Label,
      credits: tierSrc?.tier4Credits || dv.tier4Credits,
      value: tierSrc?.tier4Value || dv.tier4Value,
    },
  ];
  ...
  const subStyles: { id: SubStyle; name: string; class: string; glow: string; textClass: string }[] = [
    {
      ...
      name: "GOLD",
      class: "bg-amber-400",
      glow: "shadow-[0_0_20px_rgba(251,191,36,0.6)] border-amber-500/30",
      textClass: "text-amber-400 drop-shadow-[0_0_12px_rgba(251,191,36,1)]",
    },
    {
      id: "BLUE",
      name: "BLUE",
      class: "bg-cyan-400",
      glow: "shadow-[0_0_20px_rgba(34,211,238,0.6)] border-cyan-500/30",
      textClass: "text-cyan-400 drop-shadow-[0_0_12px_rgba(34,211,238,1)]",
    },
    {
      id: "RUBY",
      name: "RUBY",
      class: "bg-rose-400",
      glow: "shadow-[0_0_20px_rgba(244,63,94,0.6)] border-rose-500/30",
      textClass: "text-rose-400 drop-shadow-[0_0_12px_rgba(244,63,94,1)]",
    },
    {
      id: "EMERALD",
      name: "EMERALD",
      class: "bg-emerald-400",
      glow: "shadow-[0_0_20px_rgba(16,185,129,0.6)] border-emerald-500/30",
      textClass: "text-emerald-400 drop-shadow-[0_0_12px_rgba(16,185,129,1)]",
    },
    {
      id: "DIAMOND",
      name: "DIAMOND",
      class: "bg-white",
      glow: "shadow-[0_0_20px_rgba(255,255,255,0.4)] border-white/20",
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <header className="p-4 flex items-center gap-3 bg-card border-secondary-foreground">
        ...
        {isEditing ? (
          ...
          <h1
            className="font-bold tracking-wider text-4xl text-white"
            style={{
      ...
      <main className="px-4 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700 border-gold bg-blue-950">
        ...
        {isEditing && (
          <div className="bg-purple-900/20 border border-purple-500/20 rounded-xl p-3 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <input
                value={editData?.appApiBtnText || ""}
                onChange={(e) => setEditData({ ...editData!, appApiBtnText: e.target.value })}
                className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                placeholder="App API text"
              />
              <input
                value={editData?.ownApiBtnText || ""}
                onChange={(e) => setEditData({ ...editData!, ownApiBtnText: e.target.value })}
                className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                placeholder="Own API text"
              />
            </div>
          </div>
        )}
          ...
          <div className="bg-white/5 backdrop-blur-2xl rounded-[28px] p-6 border border-white/10 space-y-3 shadow-xl animate-in zoom-in-95 duration-300">
            {isEditing ? (
              ...
              <h4
                ...
                style={{ color: vs?.apiKeyLabelColor || defaultVoiceSettings.apiKeyLabelColor }}
              >
        ...
        <div className="bg-white/5 backdrop-blur-2xl rounded-[28px] p-5 border border-white/10 space-y-3 shadow-xl text-destructive-foreground">
          <div className="flex items-center gap-2 px-1 text-platinum bg-card">
            ...
            {isEditing ? (
              ...
              <h4
                ...
                style={{ color: vs?.languageLabelColor || defaultVoiceSettings.languageLabelColor }}
              >
          ...
          <Select value={selectedLanguage} onValueChange={setSelectedLanguage}>
            ...
            <SelectContent className="max-h-[300px] bg-background border-white/10">
              {languages.map((lang) => (
                <SelectItem key={lang.code} value={lang.code} className="cursor-pointer hover:bg-white/10">
        ...
        <div className="bg-white/5 backdrop-blur-2xl rounded-[28px] p-5 border border-white/10 space-y-3 shadow-xl">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-[12px]">
              {isEditing ? (
                ...
                <label
                  className="font-black uppercase tracking-widest text-base"
                  style={{ color: vs?.scriptLabelColor || defaultVoiceSettings.scriptLabelColor }}
                >
            ...
            {!isEditing && (
              <div className="flex gap-4">
                ...
                >
                  {vs?.pasteBtnText || defaultVoiceSettings.pasteBtnText}
                </button>
                <button
                  onClick={() => {
                    setText("");
                  ...
                  className="text-[9px] font-black text-destructive uppercase tracking-widest"
                >
                  {vs?.clearBtnText || defaultVoiceSettings.clearBtnText}
                </button>
              </div>
            )}
          </div>
          {isEditing && (
            <div className="grid grid-cols-2 gap-2">
              <input
                value={editData?.pasteBtnText || ""}
                onChange={(e) => setEditData({ ...editData!, pasteBtnText: e.target.value })}
                className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                placeholder="Paste btn"
              />
              <input
                value={editData?.clearBtnText || ""}
                onChange={(e) => setEditData({ ...editData!, clearBtnText: e.target.value })}
                className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
        ...
        <div className="space-y-4">
          <div className="space-y-2 text-neon-cyan bg-violet-900">
            {isEditing ? (
              <EditableField fieldKey="performanceLabel" colorKey="performanceLabelColor" label="Performance Label" />
            ) : (
              <h4
                className="font-black uppercase tracking-widest ml-1 text-base text-neon-rose"
                style={{ color: vs?.performanceLabelColor || defaultVoiceSettings.performanceLabelColor }}
              >
                {vs?.performanceLabel || defaultVoiceSettings.performanceLabel}
              </h4>
            )}
            <div className="grid grid-cols-4 gap-1.5 text-gold-dark">
              {["EXCITING", "CALM", "PROFESSIONAL", "NARRATIVE"].map((perf) => (
                <button
                  key={perf}
                  onClick={() => setPerformance(perf)}
                  className={`py-2 rounded-[10px] font-black text-[7px] uppercase tracking-tighter transition-all border ${performance === perf ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white border-transparent shadow-sm" : "bg-white/5 border-white/5 text-muted-foreground"}`}
                >
                  {perf}
                </button>
              ))}
          ...
          <div className="space-y-2">
            {isEditing ? (
              ...
              <h4
                className="font-black uppercase tracking-widest ml-1 text-base text-neon-rose"
                style={{ color: vs?.characterLabelColor || defaultVoiceSettings.characterLabelColor }}
              >
                {vs?.characterLabel || defaultVoiceSettings.characterLabel}
              </h4>
            )}
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide px-0.5 text-neon-rose bg-accent-foreground">
              {voices.map((v, idx) => (
                <button
                  key={`${v.name}-${idx}`}
                ...
                >
                  <span
                    className={`font-black text-[11px] tracking-tight ${voiceName === v.name ? "text-white" : "text-foreground/60"}`}
                  >
                    {v.name}
                  </span>
                  <span className={`text-[6px] font-black ${voiceName === v.name ? "opacity-80" : "opacity-40"}`}>
                    {v.gender}
                  </span>
                </button>
              ))}
        ...
        {!showOptions ? (
          <>
            {isEditing && <EditableField fieldKey="checkWordBtnText" label="Check Word Button Text" />}
            <button
              disabled={!text}
              onClick={handleCheckWordCount}
              className="w-full py-4 rounded-[20px] bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 disabled:opacity-30"
            >
          ...
          <div className="space-y-4 animate-in fade-in zoom-in-95 duration-500">
            <div className="bg-white/5 backdrop-blur-2xl rounded-[24px] p-4 border border-white/10 flex justify-between items-center">
              <div className="space-y-0.5">
                {isEditing ? (
                  ...
                  <>
                    <h4 className="font-black text-foreground uppercase tracking-widest text-base">
                      {vs?.proSubtitleTitle || defaultVoiceSettings.proSubtitleTitle}
                    </h4>
                    <p
                      className="text-[7px] font-black uppercase tracking-widest"
                      style={{ color: vs?.proSubtitleSubColor || defaultVoiceSettings.proSubtitleSubColor }}
                    >
                      {vs?.proSubtitleSub || defaultVoiceSettings.proSubtitleSub}
                    </p>
                  </>
                )}
              </div>
              <button
                onClick={() => setProSubtitles(!proSubtitles)}
                className={`w-10 h-6 rounded-full p-1 transition-all duration-300 ${proSubtitles ? "bg-primary" : "bg-muted"}`}
              >
                <div
                  className={`w-4 h-4 rounded-full bg-white transition-transform duration-300 ${proSubtitles ? "translate-x-4" : "translate-x-0"}`}
                ></div>
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {tiers.map((tier) => (
                <button
                  ...
                  disabled={tierLocked}
                  onClick={() => {
                    if (!tierLocked) setSelectedTier(tier.value);
                  }}
            ...
            {isEditing && (
              <div className="space-y-2 p-3 rounded-xl border border-white/10 bg-black/30">
                <p className="text-[8px] font-black text-amber-400 uppercase tracking-widest">
                  ✏️ EDIT TIERS (Label / Credits Text / Char Value)
                </p>
                {[1, 2, 3, 4].map((i) => (
            ...
            {isEditing && (
              <div className="grid grid-cols-2 gap-2">
                <input
                  value={editData?.generateAudioOnlyText || ""}
                  onChange={(e) => setEditData({ ...editData!, generateAudioOnlyText: e.target.value })}
                  className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                  placeholder="Audio Only text"
                />
                <input
                  value={editData?.generateWithSubsText || ""}
                  onChange={(e) => setEditData({ ...editData!, generateWithSubsText: e.target.value })}
                  className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                  placeholder="With Subs text"
                />
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-4 rounded-[20px] bg-foreground text-background font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all"
            >
              {loading
                ? "PROCESSING..."
                : proSubtitles
                  ? vs?.generateWithSubsText || defaultVoiceSettings.generateWithSubsText
                  : vs?.generateAudioOnlyText || defaultVoiceSettings.generateAudioOnlyText}
            </button>
          </div>
        )}
        ...
        {resultAudio && proSubtitles && (
          <div className="animate-in fade-in zoom-in-95 duration-700 space-y-3">
            <div
              className={`bg-black/95 rounded-[36px] p-7 border-2 transition-all duration-700 relative overflow-hidden flex flex-col items-center justify-center text-center min-h-[140px] ${currentStyle.glow}`}
            >
              <div className="absolute top-4 right-5 flex gap-1">
                {subStyles.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => setActiveSubStyle(s.id)}
                    className={`w-3 h-3 rounded-full border border-white/20 transition-transform ${activeSubStyle === s.id ? "scale-110 border-white ring-1 ring-white/10" : "opacity-20"} ${s.class}`}
                  />
                ))}
              </div>
              <div className="absolute top-4 left-5 flex items-center gap-1.5">
                ...
                <span className="text-[7px] font-black text-muted-foreground uppercase tracking-widest">
                  {isPlaying ? "LIVE SUBTITLE" : "PREVIEW"}
                </span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2 animate-in fade-in duration-300">
            {isEditing && (
              <div className="col-span-2 grid grid-cols-2 gap-2 mb-1">
                <input
                  value={editData?.downloadAudioText || ""}
                  onChange={(e) => setEditData({ ...editData!, downloadAudioText: e.target.value })}
                  className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                  placeholder="Download Audio text"
                />
                <input
                  value={editData?.downloadSrtText || ""}
                  onChange={(e) => setEditData({ ...editData!, downloadSrtText: e.target.value })}
                  className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                  placeholder="Download SRT text"
                />
              </div>
            )}
            <button
        ...
        <div className="space-y-3 pt-2">
          <div className="flex justify-between items-center px-1">
            {isEditing ? (
              ...
              <h4
                className="font-black uppercase tracking-widest text-neon-rose text-lg"
                style={{ color: vs?.historyTitleColor || defaultVoiceSettings.historyTitleColor }}
              >
            ...
            {history.length > 0 && !isEditing && (
              <button
                ...
                className="text-[8px] font-black text-destructive uppercase tracking-widest border border-destructive/20 px-2 py-1 rounded-lg hover:bg-destructive/10 transition-colors"
              >
          ...
          {isEditing && (
            <div className="grid grid-cols-2 gap-2">
              <input
                value={editData?.clearHistoryText || ""}
                onChange={(e) => setEditData({ ...editData!, clearHistoryText: e.target.value })}
                className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                placeholder="Clear History btn"
              />
              <input
                value={editData?.noHistoryText || ""}
                onChange={(e) => setEditData({ ...editData!, noHistoryText: e.target.value })}
                className="bg-black/40 border border-white/10 rounded-lg p-2 text-[10px] font-black text-white outline-none"
                placeholder="No history text"
              />
            </div>
          )}
          ...
          {history.length === 0 ? (
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {history.map((item) => (
                <div
                  key={item.id}
                  className="bg-white/5 border border-white/5 p-4 rounded-2xl flex items-center justify-between group animate-in slide-in-from-right-2 duration-300"
                >
                  <div
                    className="flex-1 min-w-0 pr-4 cursor-pointer"
                    onClick={() =>
                      item.audio
                        ? playHistoryItem(item.audio, item.text)
                        : alert("ဤမှတ်တမ်းတွင် အသံဖိုင်မရှိတော့ပါ။ (Session ပြီးဆုံးပြီ)")
                    }
                  >
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        item.audio ? playHistoryItem(item.audio, item.text) : alert("ဤမှတ်တမ်းတွင် အသံဖိုင်မရှိတော့ပါ။")
                      }
                      className={`w-7 h-7 flex items-center justify-center rounded-lg transition-all ${item.audio ? "bg-primary/10 text-primary hover:bg-primary hover:text-white" : "bg-muted/20 text-muted-foreground/40 cursor-not-allowed"}`}
                    >
                      <svg
                    ...
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center bg-destructive/10 rounded-lg text-destructive hover:bg-destructive hover:text-white transition-all"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="12"
        ...
        <div className="space-y-3 pt-2">
          <div className="border border-white/10 rounded-[28px] p-6 space-y-3 shadow-inner text-gold bg-blue-950">
            <div className="flex items-center gap-2">
              ...
              {isEditing ? (
                ...
                <h4
                  className="font-black uppercase tracking-[0.2em] text-base text-neon-rose"
                  style={{ color: vs?.howToUseTitleColor || defaultVoiceSettings.howToUseTitleColor }}
                >
            ...
            {isEditing ? (
              <EditableTextarea
                fieldKey="howToUseText"
                colorKey="howToUseTextColor"
                sizeKey="howToUseTextSize"
                label="How To Use Content (\\n = new line)"
              />
            ) : (
              <div
                className="space-y-2 font-bold leading-relaxed rounded-lg shadow-lg px-0 pb-0 pr-0 bg-blue-950 text-orange-400"
                style={{
                  color: vs?.howToUseTextColor || defaultVoiceSettings.howToUseTextColor,
                  fontSize: (vs?.howToUseTextSize || defaultVoiceSettings.howToUseTextSize) + "px",
                }}
              >
          ...
          <div className="bg-amber-400/5 border border-amber-400/20 rounded-[28px] p-6 space-y-3">
            <div className="flex items-center gap-2">
              ...
              {isEditing ? (
                ...
                <h4
                  ...
                  style={{ color: vs?.proTipsTitleColor || defaultVoiceSettings.proTipsTitleColor }}
                >
            ...
            {isEditing ? (
              <EditableTextarea
                fieldKey="proTipsText"
                colorKey="proTipsTextColor"
                sizeKey="proTipsTextSize"
                label="Pro Tips Content (\\n = new line)"
              />
            ) : (
              <div
                className="space-y-2 font-bold leading-relaxed text-yellow-600"
                style={{
                  color: vs?.proTipsTextColor || defaultVoiceSettings.proTipsTextColor,
                  fontSize: (vs?.proTipsTextSize || defaultVoiceSettings.proTipsTextSize) + "px",
                }}
              >
        ...
        <div className="text-center pt-6 opacity-20 text-gold">
          {isEditing ? (
            ...
            <p
              className="font-black tracking-[0.4em] uppercase text-sm text-slate-50 bg-indigo-950"
              style={{ color: vs?.footerTextColor || defaultVoiceSettings.footerTextColor }}
            >
